function [Controls_u,target_theta,new_Pid_I]=state3control_refine(Model_consts,State_variables,solutions_dd,Pid_Is)
    Controls_u=[0,0];
    ...
    eq4= ddot_beta-ddot_alpha==10*(dalpha-dbeta);
    ...
    % Define cost function matrices
    Q = [120 0;
         0 120];% Penalize both x and v equally
    R = 1;       % Control effort penalty
    ...
    %test=[(double(subs(ddot_beta_sol,values))),(double(subs(ddot_alpha_sol,values))),(double(subs(tau_2_sol,values)))]
    kk=(double(subs(tau_1_sol,values)));
    %disp(kk);
    Controls_u(1)=-( P +I+ D)+kk(2);%+(double(subs(tau_1_sol,values)));
