...
results_noised = cell(num_states, 1);
...
for i = 1:num_states
    ...
    [t, COMs, COMvs, betas, thetas, target_thetas] = main_video_noised(init_state);
    
    % Store the results in the cell array
    %results_noised{i} = struct('t', t, 'COMs', COMs, 'COMvs', COMvs, 'betas', betas, 'thetas', thetas, 'target_thetas', target_thetas);
end

% Optionally, save the results to a MAT-file
%save('results_noised.mat', 'results_noised');
