function [ddot_theta_sol, ddot_alpha_sol, ddot_beta_sol]=Zsolve_dd_disturb()
    ...
    eq1 = -ddot_x2*(cos(beta)*l2 + cos(alpha)*l1)*m2 - (ddot_y2 + g)*(sin(beta)*l2 + sin(alpha)*l1)*m2 - ddot_x1*cos(alpha)*l1*m1 - (ddot_y1 + g)*sin(alpha)*l1*m1 == tau1-F_external*l1;
    ...
    eq3 = Nx*(ddot_x0*m0 + ddot_x1*m1 + ddot_x2*m2-F_external) + Ny*((ddot_y0 + g)*m0 + (ddot_y1 + g)*m1 + (ddot_y2 + g)*m2) == Nx^2 + Ny^2;

    % Solve the equations
    [ddot_theta_sol, ddot_alpha_sol, ddot_beta_sol] = solve([eq1, eq2, eq3], [ddot_theta, ddot_alpha, ddot_beta]);
    
    
    % Simplify the solutions BEFORE calling subs()
    ddot_theta_sol = simplify(ddot_theta_sol);
    ddot_alpha_sol = simplify(ddot_alpha_sol);
    ddot_beta_sol = simplify(ddot_beta_sol);
    %{
